/*global history */
sap.ui.define([
	"ZINE/controller/BaseController",
	...
	return BaseController.extend("ZINE.controller.Master", {
		...
		onInit: function() {
			...
			this._oGroupSortState = new GroupSortState(oViewModel, grouper.Lneam(this.getResourceBundle()));
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("Chtam", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("ZINE.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("Lneam", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("Lneam", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							Prnum: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Prnum"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				Prnum: encodeURIComponent(oItem.getBindingContext().getProperty("Prnum"))
			}, bReplace);
		},
