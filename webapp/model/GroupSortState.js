sap.ui.define([
	...
	return BaseObject.extend("ZINE.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias ZINE.model.GroupSortState
		 */
		...
		/**
		 * Groups by Lneam, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function(sKey) {
			...
			if (sKey === "Lneam") {
				...
				this._oViewModel.setProperty("/sortBy", "Lneam");

				aSorters.push(
					new Sorter("Lneam", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Chtam");
			}
